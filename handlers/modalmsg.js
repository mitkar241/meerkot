...
function modalmsg(event) {
  logger.debug({
    message: "modal message triggered", user: event.user, channel: event.channel, text: event.text,
  });
  ...
  try {
    ...
    logger.error({
      message: "posting modal message failed", err: error, user: event.user, channel: event.channel, channel_type: event.channel_type,
    });
  }
}
