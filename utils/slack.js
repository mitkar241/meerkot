    ...
    */
    ...
    parsePayload(type, payloadObj) {
        try {
            ...
            logger.error({
                message: "parsing message payload failed", type: "message", err: error, payloadObj: payloadObj,
            });
            return {};
        }
    }
    ...
    postMessage(payloadObj) {
        try {
            ...
            logger.error({
                message: "posting message failed", type: "message", err: error, payloadObj: payloadObj,
            });
        }
    }
    ...
    postEphemeral(payloadObj) {
        try {
            ...
            logger.error({
                message: "posting message failed", type: "ephemeral", err: error, payloadObj: payloadObj,
            });
        }
    }
    ...
    */
    ...
    viewspublish(viewObj) {
        try {
            ...
            logger.error({
                message: "publishing view failed", err: error, viewObj: viewObj,
            });
        }
    }
    ...
    */
    ...
    viewsopen(viewObj) {
        try {
            ...
            logger.error({
                message: "opening view failed", err: error, viewObj: viewObj,
            });
        }
    }
    ...
    */
    ...
    filesupload(fileObj) {
        try {
            ...
            logger.error({
                message: "uploading file failed", err: error, fileObj: fileObj,
            });
        }
    }
    ...
    addRxn(chanId, tstamp, emoji) {
        try {
            ...
            logger.info({
                message: "emoji added", channel: chanId, thread: tstamp, emoji: emoji,
            });
        } catch (error) {
            logger.error({
                message: "emoji addition failed", error: error, channel: chanId, thread: tstamp, emoji: emoji,
            });
        }
    }
    ...
    removeRxn(chanId, tstamp, emoji) {
        try {
            ...
            logger.info({
                message: "emoji removed", channel: chanId, thread: tstamp, emoji: emoji,
            });
        } catch (error) {
            logger.error({
                message: "emoji removal failed", error: error, channel: chanId, thread: tstamp, emoji: emoji,
            });
        }
    }
}
