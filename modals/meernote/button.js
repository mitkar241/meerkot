...
app.event('app_mention', async ({ ack, body, client, event, say }) => {
  ...
  if (event.text === `<@${config.bot.id}> note`) {
    printMeerNoteButton(ack, body, client, event, say)
  }
});

// Show Help
function printMeerNoteButton(ack, body, client, event, say) {
  try {
    say(
      {
        ...
        "blocks": [
          {
            ...
            "text": {
              "type": "mrkdwn",
              "text": "Create a note ticket using meernote"
            },
            "accessory": {
              "type": "button",
              "text": {
...
*/
app.action('meernote', async( { ack, body, client, say } ) => {
  // fired when a clicked button's actionId matches
  // Acknowledge action request before anything else
  await ack();

  logger.info({"message": "meernote ticket invoked", "user": body.user.user_name});
  try {
