...
app.event('app_mention', async ({ ack, body, client, event, say }) => {
  ...
  if (event.text === `<@${config.bot.id}> call`) {
    printMeerCallButton(ack, body, client, event, say)
  }
});

// Show Help
function printMeerCallButton(ack, body, client, event, say) {
  try {
    say(
      {
        ...
        "blocks": [
          {
            ...
            "text": {
              "type": "mrkdwn",
              "text": "Create a note ticket using meercall"
            },
            "accessory": {
              "type": "button",
              "text": {
...
*/
app.action('meercall', async( { ack, body, client, say } ) => {
  // fired when a clicked button's actionId matches
  // Acknowledge action request before anything else
  await ack();

  logger.info({"message": "meercall ticket invoked", "user": body.user.user_name});
  try {
